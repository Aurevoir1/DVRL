...
def remove_high_low(dve_out, eval_model, x_train, y_train,
    ...
    评估一处高低价值样本后的表现
    Args:参数列表
      dve_out: data values   数据价值
      eval_model: evaluation model (object)   评价model
    ...
    # Sorts samples by data values
    num_bins = 20  # Per 100/20 percentile
    sort_idx = np.argsort(dve_out)  # 由小到大
    n_sort_idx = np.argsort(-dve_out)  # 由大到小
    ...
    for itt in range(num_bins):
      # itt = 1 时， 去除了5%的低/高价值样本
      # itt = 4 时， 去除了20%的高/低价值样本

        # 1. Remove least valuable samples first  先移除低值样本
        new_x_train = x_train[sort_idx[int(itt * len(x_train[:, 0]) / num_bins):], :]
        # new_x_train  得出的结果是 将x_train从小到大排序后， 从第int(itt * len(x_train[:, 0]) / num_bins)开始
        # 到最后结束的小数据集  也就是去除了排在最前边的一部分数据  相当于去除了低价值的样本
        ...
        if len(np.unique(new_y_train)) > 1:
            ...
            if perf_metric == 'auc':
                ...
                # [:,1]表示，将所有维度的第一个元素取出来， 也就是将所有预测的结果取出来
        ...
        # 去除高价值样本， 使用的方法与去除低价值样本一样  只是使用的排序标签是倒置的。 （由大到小的）
